...
// prototype and function ast node
#[derive(Debug, PartialEq, Clone)]
...
impl <'a> Parser<'a> {
    ...
    fn parse_primary(&mut self) -> Result<Box<Expr>, String> {
        match self.current {
            ...
            Some(Token::UnknownChar('(')) => self.parse_paren_expr(),
    ...
    fn parse_bin_op_rhs(&mut self, prec: u32, mut lhs: Box<Expr>) -> Result<Box<Expr>, String> {
        loop {
            ...
            let next_prec = match self.current {
                Some(Token::UnknownChar(c)) => token_precedence(c),
                _ => None,
            };
            match next_prec {
    ...
    pub fn parse_prototype(&mut self) -> Result<Prototype, String> {
        ...
        self.get_next_token();
        if self.current != Some(Token::UnknownChar('(')) {
        ...
        loop {
            self.get_next_token();
            match self.current {
                Some(Token::Identifier(ref arg_name)) => {
